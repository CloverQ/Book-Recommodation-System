<template>
    <div>
        ...
        <a-form :model="formState" name="validate_other" v-bind="formItemLayout" @finishFailed="onFinishFailed"
            ...
            <a-form-item label="Nickname" name="nickname" :rules="[{ required: false }]">
                <a-input v-model:value="users.nickName" />
            </a-form-item>

            <a-form-item label="Email" name="email" :rules="[{ required: false }]">
                <a-input v-model:value="users.email" />
            </a-form-item>

            <a-form-item label="Gender" name="gender" :rules="[{ required: false }]">
                <a-input v-model:value="users.gender" />
            </a-form-item>

            <a-form-item label="avatar" name="email" :rules="[{ required: false }]">
                <a-input v-model:value="users.avatar" />
            </a-form-item>
...
export default defineComponent({
    ...
    methods: {
        onSubmit() {
            ...
            axios.post('users/save', this.users)
                .then(response => {
